...
from models import Video, AppliedLabel, Frame, Region, Query, QueryResults, TEvent, IndexEntries, VDNDataset, VDNServer, \
    Tube, Clusters, ClusterCodes, Segment
...
class VideoExportSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Video
        fields = (
            'name', 'length_in_seconds', 'height', 'width', 'metadata', 'frames', 'created', 'description', 'uploaded',
            'dataset',
            'uploader', 'segments', 'url', 'youtube_video', 'frame_list', 'segment_list', 'event_list', 'label_list',
            'tube_list', 'index_entries_list')
